...
plot_vaf_dist <- function(p_dat) {
  alt_depth_bins <- c("0", "1", ">1", ">5", ">10", ">50")
  p_dat2 <-
    p_dat %>%
    mutate(alt_vaf = alt_depth / total_depth,
           alt_depth_bin = case_when(alt_depth > 50 ~ ">50",
                                     alt_depth > 10 ~ ">10",
                                     alt_depth > 5 ~ ">5",
                                     alt_depth > 1 ~ ">1",
                                     alt_depth == 1 ~ "1",
                                     alt_depth == 0 ~ "0") %>%
             factor(levels = alt_depth_bins))
  p_dat2 %>%
    ggplot(aes(x = alt_vaf, fill = alt_depth_bin, colour = alt_depth_bin)) +
    geom_histogram(bins = 100) +
    geom_vline(xintercept = median(p_dat2$alt_vaf, na.rm = TRUE),
               linetype = "dashed") +
    theme_minimal() +
...
purrr::walk2(names(dat$geno), dat$geno, function(p_source, p_dat) {
  ...
  p <-
    p_dat %>%
    ggplot(aes(x = alt_vaf, colour = id)) +
    geom_density() +
  ...
  seq(1, 10) %>%
    purrr::walk(function(min_alt_depth) {
      p <-
        p_dat %>%
        filter(alt_depth >= min_alt_depth) %>%
        plot_vaf_dist() +
        ggtitle(paste(p_source, "- Distribution of VAFs, min", min_alt_depth,
                      "alt depth"))
      print(p)
    })
})
...
plot_vaf_heatmap <- function(p_dat, p_source) {
  ...
  p_dat2 <-
    p_dat %>%
    mutate(alt_id = paste(chr, pos, ref, alt, sep = "_")) %>%
    # count number of cells with each mutation
    add_count(alt_id, name = "n_cells_w_mut")

  # reshape data for heatmap
  heatmap_data <-
    p_dat2 %>%
    reshape2::dcast(alt_id + n_cells_w_mut ~ id, value.var = "alt_vaf")
  rownames(heatmap_data) <- heatmap_data$alt_id
  heatmap_matrix <- as.matrix(heatmap_data[, -c(1:3)]) # drop annotations
  n_total_muts <- n_distinct(p_dat2$alt_id)
  ...
  rownames(annotation_row) <- heatmap_data$alt_id
...
purrr::walk2(names(dat$geno), dat$geno, function(p_source, p_dat) {
  p <-
    p_dat %>%
    filter(alt_depth > 5, alt_vaf > 0.1) %>%
    plot_vaf_heatmap(p_source)
  print(p)
  p <-
    p_dat %>%
    filter(alt_depth > 5, alt_vaf > 0.1, !is.na(gene)) %>%
...
purrr::walk2(names(dat$geno), dat$geno, function(p_source, p_dat) {
  ...
  purrr::walk2(names(p_dat_by_id), p_dat_by_id, function(p_id, p_dat2) {
    p <-
      p_dat2 %>%
      select(id, chr, pos, alt_vaf) %>%
      mutate(alt_baf = 1 - alt_vaf,
